...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        scores_dict_item = psychmGVGP_model.train_one_epoch(dataloaders_dict['train'], optimizer)
        scores_dict['train'].update(scores_dict_item)
        all_scores_dict['train']['epochs'].append(epoch)
        
        if epoch % epoch_block == 0:
            for split in ['val', 'test']:
                scores_dict_item = psychmGVGP_model.validate(dataloaders_dict[split], loss_fn=psychmGVGP_model.loss_fn)
