...
def SET_DEFAULT_SEARCH_SPACE():
    default_search_space = {
        ...
        "epoch": ray.tune.choice(range(5, 100 + 1)),
...
def main(config=None, num_samples=100, max_num_epochs=200, gpus_per_trial=0, results_dir=None, random_state=None, tune=False):
    # Set the random state for reproducibility if provided
    ...
    result = ray.tune.run(
        ray.tune.with_parameters(LPU.scripts.uPU.run_uPU.train_model, dataloaders_dict=dataloaders_dict, with_ray=True),
        ...
        callbacks=[LPU.utils.ray_utils.PlotMetricsCallback(allow_list=['time_this_iter_s', 'learning_rate', 'gamma', 'beta', 'val_overall_loss', 'val_y_APS', 'val_y_accuracy', 'val_y_auc'], json_save_dir=json_save_dir)],
        )
    ...
    best_model = LPU.models.uPU.uPU.uPU(config=best_model_checkpoint["config"], 
                                         dim=dataloaders_dict['train'].dataset.X.shape[-1])
    best_model.load_state_dict(best_model_checkpoint["model_state"])

    loss_fn = LPU.models.uPU.uPU.uPUloss(prior=best_model.prior,
