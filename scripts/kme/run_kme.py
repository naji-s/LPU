...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        scores_dict_item = kme_model.train_one_epoch(optimizer=optimizer, dataloader=dataloaders_dict['train'], holdout_dataloader=dataloaders_dict['holdout'])
        scores_dict['train'].update(scores_dict_item)
        all_scores_dict['train']['epochs'].append(epoch)

        if epoch % epoch_block == 0:
            for split in ['val', 'test']:
                scores_dict_item = kme_model.validate(dataloaders_dict[split], loss_fn=kme_model.loss_fn)
